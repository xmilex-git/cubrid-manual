...
인덱스 스캔은 논리프 노드, 리프 노드, 그리고 힙 영역을 순차적으로 탐색하는 방식으로 이루어진다.
스캔은 먼저 루트 노드를 포함한 **논리프 노드(logical internal nodes)**\를 탐색하여, **리프 노드(leaf node)**\의 시작 지점을 찾는 것으로 시작된다.

그다음, 리프 노드를 순차적으로 탐색하며 조건을 만족하는 키의 OID(Object Identifier) 정보를 조회한다.
필요한 경우, 이 OID 정보를 기반으로 **힙 영역(heap area)**\을 스캔하여 실제 데이터를 가져온다.

아래 예제는 이러한 인덱스 스캔 과정이 어떻게 수행되는지를 보여주고 있다.
...
**FUNC**: 현재 질의와 서브 질의에서 호출한 저장 함수의 결과를 누적하여 출력한다. fetch와 ioread는 저장 함수 내 질의 수행 시에 발생한 횟수는 포함하지 않는다.

*   time: 저장 함수 전체 수행 시간(ms)
*   fetch: 저장 함수 호출 중 페이지를 fetch한 횟수.
*   ioread: 저장 함수 호출 중 발생한 전체 I/O 읽기 횟수. 데이터를 읽을 때 물리적으로 디스크에 접근한 횟수
*   calls: 저장 함수 호출 횟수
...
**size**\가 설정해둔 값을 초과하는 경우에는 서브 쿼리 캐시가 질의 수행 도중 비활성화 되며 SQL 트레이스 정보에서 **status**\가 disabled로 출력된다. 
또한, **hit** 비율이 90% 미만일 경우, 서브 쿼리 캐시의 메모리 크기가 설정해둔 값을 초과하지 않더라도 캐시 miss가 높다고 판단해서 질의 수행 도중 캐시가 비활성화 될 수 있다. 
