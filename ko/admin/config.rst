...
|                               +-------------------------------------+-------------------------+---------+----------+--------------------------------+-----------------+
|                               | dblink_auto_commit                  | 서버                    |         | bool     | yes                            |                 |
+-------------------------------+-------------------------------------+-------------------------+---------+----------+--------------------------------+-----------------+
...
|                               +-------------------------------------+-------------------------+---------+----------+--------------------------------+-----------------+
|                               | json_max_array_idx                  | 서버                    | O       | int      | 65,536                         | 가능            |
|                               +-------------------------------------+-------------------------+---------+----------+--------------------------------+-----------------+
|                               | max_query_per_tran                  | 서버                    |         | int      | 100                            |                 |
|                               +-------------------------------------+-------------------------+---------+----------+--------------------------------+-----------------+
...
|                               +-------------------------------------+-------------------------+---------+----------+--------------------------------+-----------------+
|                               | stored_procedure_return_numeric_size| 클라이언트/서버         |         | list     | 38,15                          |                 |
|                               +-------------------------------------+-------------------------+---------+----------+--------------------------------+-----------------+
...
|                               +-------------------------------------+-------------------------+---------+----------+--------------------------------+-----------------+
|                               | max_parallel_workers                | 서버                    |         | int      | 32                             |                 |
|                               +-------------------------------------+-------------------------+---------+----------+--------------------------------+-----------------+
|                               | parallel_heap_scan_threads          | 서버                    |         | int      | 2                              |                 |
+-------------------------------+-------------------------------------+-------------------------+---------+----------+--------------------------------+-----------------+
...
**dblink_auto_commit**
    ...
    **dblink_auto_commit**\은 분산 트랜잭션 처리 시, dblink를 통한 질의의 커밋 방식에 영향을 주는 설정으로, 기본 값은 **yes**\이다.
        *    이 파라미터 값을 **yes**\로 설정하면, dblink 질의는 실행 즉시 자동으로 커밋된다.
        *    반대로 **no**\로 설정하면, dblink를 포함한 전체 트랜잭션이 커밋될 때 dblink를 통해 연결된 원격 서버의 질의도 함께 커밋된다.

    분산 트랜잭션에서는 dblink 질의부터 먼저 커밋을 시도한다. 만약 이 과정에서 하나라도 커밋에 실패하는 경우 전체 트랜잭션은 취소(rollback)된다.
 
    .. warning:: 

        *    일부 dblink 질의가 이미 커밋된 이후에 전체 트랜잭션 커밋이 실패하더라도, 해당 커밋은 되돌릴 수 없다.
        *    dblink 트랜잭션에서 savepoint는 지원하지 않는다.
...
+--------------------------------------+--------+------------+------------+------------+
| max_query_tran                       | int    | 100        | 1          | 32,767     |
+--------------------------------------+--------+------------+------------+------------+
...
+--------------------------------------+--------+------------+------------+------------+
| stored_procedure_return_numeric_size | list   | 38,15      | 1,0        | 38,38      |
+--------------------------------------+--------+------------+------------+------------+
...
**max_query_per_tran**
    ...
    **max_query_per_tran** 은 트랜잭션 당 최대 보유할 수 있는 질의(close되지 않은 질의) 개수를 설정시 사용되는 파라미터이다.

    기본 설정 값은 100이며, 최소값은 1,  그리고 최대값은 32,767이다.
    이 값을 너무 크게 설정하면, 사용하는 메모리가 증가하여 전체 시스템 성능에 영향을 줄 수 있으므로 주의해야 한다.
...
+---------------------------------------+--------+-------------------+---------+---------+
| max_parallel_workers                  | int    | 32                | 0       | 128     |
+---------------------------------------+--------+-------------------+---------+---------+
| parallel_heap_scan_threads            | int    | 2                 | 0       | 64      |
+---------------------------------------+--------+-------------------+---------+---------+
...
**max_parallel_workers**
    ...
    **max_parallel_workers**\는 병렬 처리 작업에 사용할 수 있는 최대 스레드 개수를 설정하는 파라미터이다. 기본값은 32이다.

**parallel_heap_scan_threads**

    **parallel_heap_scan_threads**\는 병렬 힙 스캔 작업에 사용할 수 있는 최대 스레드 개수를 설정하는 파라미터이다. 기본값은 2이다.
...
**ACCESS_LIST**
    ...
    **ACCESS_LIST** 는 CUBRID 브로커로 접근을 허용하는 응용 클라이언트의 IP 주소 리스트를 저장할 파일 이름을 지정하는 파라미터이다. 192.168.10.*와 192.168.20.*인 IP 주소의 접근을 허용하려면 이를 임의의 파일(ip_lists.txt)에 저장하여 이 파라미터의 값으로 파일명을 설정한다.
